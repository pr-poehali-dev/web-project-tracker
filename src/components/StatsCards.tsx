  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
      <Card className="border-2 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-green-50 to-green-100">
        ...
        <CardContent>
          <div className="text-2xl font-bold text-green-700">{totalRevenue.toLocaleString('ru-RU')} ₽</div>
      ...
      <Card className="border-2 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-pink-50 to-pink-100">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
          <CardTitle className="text-xs font-medium text-muted-foreground">Затраты</CardTitle>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-pink-700">{totalExpensesSum.toLocaleString('ru-RU')} ₽</div>
      ...
      <Card className={`border-2 hover:shadow-lg transition-all duration-300 ${totalMargin >= 0 ? 'bg-gradient-to-br from-emerald-50 to-emerald-100' : 'bg-gradient-to-br from-red-50 to-red-100'}`}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
          <CardTitle className="text-xs font-medium text-muted-foreground">Маржа</CardTitle>
          <div className={`h-8 w-8 rounded-full ${totalMargin >= 0 ? 'bg-emerald-500' : 'bg-red-500'} flex items-center justify-center`}>
            <Icon name={totalMargin >= 0 ? "TrendingUp" : "TrendingDown"} className="h-4 w-4 text-white" />
          </div>
        </CardHeader>
        <CardContent>
          <div className={`text-2xl font-bold ${totalMargin >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
      ...
      <Card className="border-2 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-blue-50 to-blue-100">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
          <CardTitle className="text-xs font-medium text-muted-foreground">Активные проекты</CardTitle>
          <div className="h-8 w-8 rounded-full bg-blue-500 flex items-center justify-center">
            <Icon name="FolderOpen" className="h-4 w-4 text-white" />
          </div>
        </CardHeader>
        <CardContent>
      ...
      <Card className="border-2 hover:shadow-lg transition-all duration-300 bg-gradient-to-br from-purple-50 to-purple-100">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1">
          <CardTitle className="text-xs font-medium text-muted-foreground">Клиенты</CardTitle>
